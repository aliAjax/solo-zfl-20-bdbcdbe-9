...
支持开多个服务进程共用同一数据文件：读写均由数据目录下的 **`db.json.lock/` 文件锁**互斥，同一作业在多进程下也只会被一个工作者持有。
...
| `DB_FILE` | `data/db.json` | JSON 数据文件路径；多个进程共用同一文件时靠文件锁互斥 |
| `LEASE_MS` | `30000` | 作业默认租约时长（毫秒），认领时可用 `leaseMs` 覆盖 |
| `MAX_ATTEMPTS` | `3` | 最大投递次数（认领次数，含首次），用尽转死信 |
| `LOCK_STALE_MS` | `30000` | 持锁超过此时长且持锁进程失活，视为僵死锁可被回收 |
| `LOCK_TIMEOUT_MS` | `15000` | 等待文件锁的上限，超时返回 500 |
| `CRASH_SENTINEL` | 无 | 测试用：指向一个文件路径，该文件存在时下一次写事务落盘前进程 `SIGKILL` 自杀 |
| `FAIL_PERSIST_SENTINEL` | 无 | 测试用：文件存在时落盘在 rename 前失败，验证内存回滚 |
...
使用内置 `node:test` + 全局 `fetch`，共 25 个用例，含：

- checksum 去重、作业自动创建、状态流转与结果唯一；
- 租约：唯一认领、过期抢占、旧持有者提交被拒；
- 失败重试与死信；按状态/拓片查询与统计；
- **多进程互斥**：两个真实服务进程共用一份数据文件并发认领/登记，
  同一作业恰好被一个工作者认领、队列被不重不漏抽干；
- **投递次数封顶**：假时钟反复触发租约过期再认领，到 `MAX_ATTEMPTS` 后自动转死信；
- **落盘失败回滚**：注入 rename 前故障，接口 500 且内存回滚（读不到未落盘变更），恢复后重提成功；
...
- 投递次数封顶：过期重领也计入 `attempts`。租约过期且 `attempts >= maxAttempts` 的作业
  会在下一次认领请求里**自动转死信**（响应中 `deadLettered` 为本次收尸数量），不会永驻处理中，也不会超投。
- 成功提交在同一事务里写入结果并置 `completed`：同一作业最多一条结果，重复完成返回 409。
- 失败后若 `attempts < maxAttempts` 回 `queued` 等待重试；达到上限转 `dead_letter`，不再被认领。

### 多进程部署

可以在同一台机器上启动多个 `node server.js` 进程并指向同一个 `DB_FILE`（如配合外部负载均衡/多工作者轮询）：

- 每次读、写事务都先抢数据目录下的 `db.json.lock/` 目录锁，事务结束才释放；
  两个进程同时认领同一作业时，后拿到锁的进程会重读磁盘并看到作业已是 `processing`，于是返回 `claimed:false`；
- 锁目录内 `owner.json` 记录持锁进程与时间戳。进程崩溃留下的锁在“进程失活”或
  持锁超过 `LOCK_STALE_MS` 后才会被回收，owner 刚创建的极短窗口内绝不强抢（避免双持）；
- 该锁保证的是**同一文件系统**上的多进程互斥（单机或共享块存储/NFS 且支持原子 mkdir/rename）。
  跨地域多活请换用真正的数据库。

### 崩溃与重启保证

- 所有状态变更走跨进程串行写事务：拿到文件锁后先从磁盘重读，同一时刻全机只有一个事务在改数据；
- 先改内存状态，再 `write 临时文件 → fsync → rename` 覆盖正式文件。落盘前被杀，磁盘保持上一个完整版本；
  落盘后被杀，结果与 completed 状态同时可见——不存在“状态完成了但没结果”或反之的半成品；
- 落盘**抛错**（磁盘满、权限、I/O 错误）时事务整体回滚：内存恢复到磁盘版本，接口返回 500，
  随后的查询读不到未落盘的变更；故障排除后用同一语义重试即可；
...
- 有可领作业：`200 {"claimed": true, "reclaimed": false, "deadLettered": 0, "data": {作业+image+result}}`；
- 抢占过期作业：`reclaimed: true`；
- 本次认领顺带把过期且用尽投递次数的作业收进死信：`deadLettered` 为数量；
- 无作业可领：`200 {"claimed": false, "reclaimed": false, "deadLettered": n, "data": null}`。
...
- `db.json.<pid>.<rand>.tmp`：原子落盘的临时文件，正常情况下立即消失，启动时自动清理遗留；
- `db.json.lock/`：跨进程互斥锁目录（内含 `owner.json`），只在事务进行的毫秒级窗口内存在。
  若进程被 `SIGKILL` 留下僵死锁，下一个进程会在确认持锁进程失活或锁超龄（`LOCK_STALE_MS`）后自动回收；
  数据文件可整体拷贝备份（备份时请在停服或确认无写事务时进行，不要只拷走 `.tmp`/`.lock`）。
